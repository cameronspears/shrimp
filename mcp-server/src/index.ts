...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description:
        'Run health checks on staged files before committing. Perfect for pre-commit hooks to ensure code quality. Checks only the files about to be committed, optionally auto-fixes issues, and enforces a minimum health score threshold. Returns commit recommendation (proceed/block) based on findings.',
      inputSchema: {
        type: 'object',
        properties: {
          stagedFiles: {
        ...
        try {
          ...
          if (!filesToCheck || filesToCheck.length === 0) {
            try {
              ...
              throw new McpError(
                ErrorCode.InternalError,
                'Failed to get staged files. Are you in a git repository?'
              );
            }
          }
          ...
          const criticalIssues = parsed.recommendations.filter(
            (r) => r.includes('[CRITICAL]') || r.includes('[ERROR]')
          );
          ...
          throw new McpError(
            ErrorCode.InternalError,
            `Pre-commit check failed: ${error instanceof Error ? error.message : 'Unknown error'}`
          );
        }
      }
